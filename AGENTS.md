...
仕様だけでは潰せない不確実性を **spike（使い捨て検証）** で確認し、学びを仕様に還元するサイクル。

### spike の定義
- spike = **「問いに答えるための最小実装」**。プロダクトコードではない。
- 目的は **仮説の検証**（動くか？成立するか？制約はあるか？）であり、機能の完成ではない。

### spike と main の境界（不可侵）

| | spike branch | main |
|---|---|---|
| 目的 | 不確実性を潰す | 品質基準を満たすコードと確定仕様のみ |
| コード品質 | 暫定可（ハードコード、TODO、テスト不足OK） | 暫定コード禁止（§6 基本ルール準拠） |
| マージ対象 | **マージしない**（spike branch はアーカイブまたは削除） | spike の学びから生まれた **DEC + 本実装コード** のみマージ |
| ci.sh | 通す必要なし | 必ず green |

### spike のライフサイクル

```
1. 問いを定義（spike backlog から選択）
   ↓
2. feature branch を切る（spike/<topic>）
   ↓
3. 最小実装で問いに答える
   ↓
4. 結果を記録（artifacts/runs/SPIKE-xxxx.out.md）
   - 問い / 結果 / 制約・発見 / 仕様への示唆
   ↓
5. 仕様還元
   - 確定事項 → DEC（docs/rfc/DEC-xxx.md）
   - 仕様変更 → docs/constitution/ の該当箇所を更新
   - OPEN 解消 → OPEN Index を更新
   ↓
6. 本実装（必要なら）
   - spike コードは使わない。仕様から本実装を書く
   - main にマージ、ci.sh green
   ↓
7. spike branch をアーカイブ（タグ付け）または削除
```

### spike 結果テンプレート（最小）
パス: `artifacts/runs/SPIKE-xxxx-<topic>.out.md`
```md
# SPIKE-xxxx: タイトル
...
- DEC化すべき事項
- OPEN の解消/変更
- docs/constitution の更新箇所

## 証跡
- branch 名、コミットハッシュ、実行ログへの参照
```

### spike を切る判断基準
以下のいずれかに該当する場合、仕様議論や本実装より先に spike を行う：
- **OS/HW制約** が成立性を左右する（例: BLEバックグラウンド動作）
- **データフォーマットの往復** が未検証（例: L0→L1 envelope の round-trip）
- **暗号学的操作** の実装可能性が未確認（例: hash chain 構築、署名検証）
- **OPEN の解決** に実装知見が必要（机上では決まらない）
