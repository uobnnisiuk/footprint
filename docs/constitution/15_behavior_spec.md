...
- **Notified（通知成立）** = 宛先（本人＋プラットフォーム）への通知が、**送達キューに"確実に登録された"時点**
...
- **OPEN-011**: 通知内容の最小セット（抑止に十分・悪用に不足）
...
| ID | 内容 |
|----|------|
| OPEN-001 | オープン探索の「存在」の粒度（エリア？時間窓？） |
| OPEN-007 | 遭遇カプセルの暗号化鍵は誰が持つか |
| OPEN-008 | サイレント遭遇中継のスパム対策 |
| OPEN-010 | 権限救助者（Authorized Rescue）の定義 |
| OPEN-011 | 通知内容の最小セット |
| OPEN-012 | プラットフォーム不在/通信断時の T2 補完方法（オフライン relay → 後で受付 等） |

※OPEN-009（link の条件）は Section 2.1 で解決済み。
※全OPEN一覧は `risks.md` を参照。
