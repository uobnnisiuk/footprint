...
上記リスクの前提には「Encounter ≒ すれ違い数秒」という暗黙の想定があった。しかし、令和6年能登半島地震の実態を振り返ると、救助に効く LS の中心は「すれ違い数秒」ではなく **「持続的近接（同一建物・同一地区に分〜時間〜日）」** である。

iOS BG BLE は「すれ違い数秒」の検出を保証できないが、「持続的近接」に対しては累積検出確率が十分に高い可能性がある。
...
**検証が必要**: この仮説は SPIKE-0003（持続的近接の検出確率測定）で実測検証する。実測結果次第で、BG Encounter の保証線を「短時間=BE / 持続近接=HIGH_PROB」に段階化する可能性がある。

詳細は `docs/rfc/RFC-0006-sustained-proximity-hypothesis.md` を参照。

#### 未解決点（TODOではなく論点）

* iOS制約下で「被災者操作ゼロ」を満たす通信・中継の成立条件をどこに置くか（スマホ単体に置けない可能性を含む）。
* "保証"と"ベストエフォート"の境界（どこまでを要件、どこからを期待値にするか）。
* **持続的近接に対する BG 検出確率は実測で十分か**（RFC-0006 / SPIKE-0003 で検証）。

* **関連**: C-005（疎での価値）, C-008（サイレント遭遇中継の難所）, IF-RELAY-001, OPEN-012, RFC-0006
