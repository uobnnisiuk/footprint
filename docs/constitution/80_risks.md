...
詳細は `docs/rfc/RFC-0003-sustained-proximity-hypothesis.md` を参照。

#### 未解決点（TODOではなく論点）

* iOS制約下で「被災者操作ゼロ」を満たす通信・中継の成立条件をどこに置くか（スマホ単体に置けない可能性を含む）。
* "保証"と"ベストエフォート"の境界（どこまでを要件、どこからを期待値にするか）。
* **持続的近接に対する BG 検出確率は実測で十分か**（RFC-0003 / SPIKE-0003 で検証）。

* **関連**: C-005（疎での価値）, C-008（サイレント遭遇中継の難所）, IF-RELAY-001, OPEN-012, RFC-0003
