...
- **不確実性の種類**: OS/HW 制約 × 持続的近接仮説（RFC-0003）の検証
- **関連**: IDEA-0001, IDEA-0002, RFC-0003, RISK-BLE-IOS-001, DEC-0008
- **背景**: DEC-0008 は BG Encounter を BE（上積み）としたが、その前提は「Encounter ≒ すれ違い数秒」だった。RFC-0003（持続的近接仮説）は「分〜時間の近接なら iOS BG でも累積検出確率が十分」と主張する。本 SPIKE はこの仮説を実測で検証する。
- **検証手段**:
...
- **mainに残すもの**:
  ...
  - RFC-0003 仮説の判定結果（Accept / Reject / 条件付き）
...
SPIKE-0003 (持続的近接 BLE 実測)  ← 最優先：プロジェクト凍結リスク解除
    ↓ 結果次第で RFC-0003 Accept/Reject → DEC-0008 更新判断
    ↓
...
**変更点**: SPIKE-0003 を最優先に引き上げ。持続的近接仮説（RFC-0003）の検証結果がプロダクト方針の確定に直結するため。SPIKE-0001/0002 との並行実行を推奨。
