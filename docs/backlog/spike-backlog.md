...
| ~~OPEN-007~~ | ~~遭遇カプセルの暗号化鍵は誰が持つか~~ | **解決済み** → DEC-0004（Authorized Rescue 公開鍵） |
| ~~OPEN-011~~ | ~~通知内容の最小セット~~ | **解決済み** → DEC-0005（行為の事実 + 責任追跡 + 粗いスコープ） |
...
### SPIKE-0003: 持続的近接の BG BLE 検出確率（iOS/Android 実機）

- **問い**: iOS/Android の BG 状態で、持続的近接（数分〜数時間）における BLE 検出確率は救助用途に十分か？
- **不確実性の種類**: OS/HW 制約 × 持続的近接仮説（RFC-0006）の検証
- **関連**: IDEA-0001, IDEA-0002, RFC-0006, RISK-BLE-IOS-001, DEC-0008
- **背景**: DEC-0008 は BG Encounter を BE（上積み）としたが、その前提は「Encounter ≒ すれ違い数秒」だった。RFC-0006（持続的近接仮説）は「分〜時間の近接なら iOS BG でも累積検出確率が十分」と主張する。本 SPIKE はこの仮説を実測で検証する。
- **検証手段**:
  1. iOS 端末 + Android 端末で BLE advertise + scan の最小アプリを作成
  2. 以下の **条件マトリクス** で測定:

  | 軸 | 値 |
  |----|-----|
  | OS 状態 | FG / BG（ロック画面）/ BG（低電力モード）|
  | 近接形態 | 同室 / 同フロア / 隣室 / 上下階 |
  | 距離 | 1m / 3m / 10m |
  | 近接時間 | 1分 / 5分 / 30分 / 2時間 |
  | 端末組合 | iOS-iOS / iOS-Android / Android-Android |

  3. 各条件で以下を記録:
     - **T_detect**: 近接開始から最初の検出までの時間
     - **P(N)**: N分間で少なくとも1回検出された割合（試行10回以上）
     - **検出回数/分**: 検出頻度の分布
     - **RSSI 分布**: 信号強度の統計
  4. 連続 BG 動作時の電池消費を測定（1時間単位、iOS/Android 各）
  5. 壁越し・上下階での誤結合（近接推定のノイズ）を記録
- **成功判定**:
  - **主判定**: iOS BG で持続的近接（5分、10m以内）の P(5) ≥ 80%
  - **補助判定**: iOS BG で持続的近接（30分、10m以内）の P(30) ≥ 95%
  - **電池判定**: BG 連続動作で電池消費が 10%/日 以下
  - **「すれ違い3秒」は測定するが成功判定には含めない**（参考データとして記録）
- **mainに残すもの**:
  - OS 制約の実測データ文書（docs/backlog/ に追記）
  - RFC-0006 仮説の判定結果（Accept / Reject / 条件付き）
  - DEC-0008 の更新が必要かどうかの判断材料
- **依存**: iOS 実機 + Android 実機が必要（エミュレータでは BLE 検証不可）
- **優先度**: **高（引き上げ）** — プロジェクト凍結リスクを解除する唯一の検証パス。L1 実装とは独立だが、プロダクト方針の確定に直結する
...
- **依存**: ~~OPEN-011~~ → DEC-0005 で解決済み。着手可能
...
```
SPIKE-0003 (持続的近接 BLE 実測)  ← 最優先：プロジェクト凍結リスク解除
    ↓ 結果次第で RFC-0006 Accept/Reject → DEC-0008 更新判断
    ↓
SPIKE-0001 (envelope round-trip)  ← 0003 と並行可能（独立）
    ↓ envelope 構造が固まる
...
**変更点**: SPIKE-0003 を最優先に引き上げ。持続的近接仮説（RFC-0006）の検証結果がプロダクト方針の確定に直結するため。SPIKE-0001/0002 との並行実行を推奨。
