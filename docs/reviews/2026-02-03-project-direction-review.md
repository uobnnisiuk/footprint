...
| 6. 実用化/採用 | 未着手 | MVP定義と「最初のユーザ」像が欠けている |
...
現在のプロジェクトは「完全な災害対応システム」の仕様を精密に書いているが、「最初にリリースできる最小の機能セット」が定義されていない。

考えられるMVP候補：

| MVP候補 | スコープ | ユーザ | 必要な層 |
|---------|---------|--------|----------|
| A: 個人用足跡記録 | 自分の位置を定期記録、一覧/地図表示 | 登山者 | L0 + 最小UI |
| B: 家族共有 | AにリアルタイムではなくLast Known Good共有を追加 | 家族 | L0 + L1（最小） |
| C: 災害時プロトタイプ | B + T3モード + オープン探索 | 自治体/消防 | L0 + L1 + L2（最小） |

> **提言**: MVP-A（個人用足跡記録）を最初のマイルストーンとして定義し、L0 + Android最小UIの組み合わせで「動くもの」を作る。これがないと、仕様の妥当性を実ユーザでテストできない。
...
| 順位 | アクション | 根拠 |
|------|-----------|------|
| 1 | **MVPスコープを1つ定義する** | 「何を作らないか」が決まらないと、仕様が永遠に広がり続ける |
| 2 | **OPEN-006, OPEN-007 を解決する** | L1実装のクリティカルパス上にある |
| 3 | **L1のデータ契約（envelope.schema.json）骨格を定義する** | L0→L1の接続点を早期に固める |
| 4 | **BLE PoCを実機で実行する** | 仕様では解決できないハードウェア/OS制約を確認 |
| 5 | **OPENに優先度/フェーズを付与する** | 全部同時に解決しようとすると停滞する |
| 6 | **競合分析を書く** | footprintの独自価値を関係者に説明できるようにする |
...
ただし、現在のリスクは**「完璧な仕様を目指して実装が進まない」**状態に陥ること。11件のOPENをすべて解決してからL1に着手するのではなく、**「解決済みの範囲で動くものを作り、動くものから学ぶ」**サイクルに移行すべきフェーズに来ている。
